...
SERVICE_NAME=es-service-tools-service
...
case "$1" in
  start)
        ...
        nohup $ES_SERVICE_ALL_EXPORT_PATH/standalone-tools-export-run.sh &> /dev/null &
        ;;
  stop)
        # Stop daemons.
        echo "🦄 Shutting down $SERVICE_NAME";
        pid=`ps ax | grep -i '/standalone-tools-service-export.py' | grep -v grep | awk '{print $1}'`
        if [ -n "$pid" ]
          then
          kill -9 $pid
         else
