...
SERVICE_NAME=uptime-service-all-service
...
case "$1" in
  start)
        ...
        # nohup $ES_SERVICE_ALL_EXPORT_PATH/standalone-uptime-export.py &> /dev/null &
        python $SCRIPTDIR/standalone-uptime-export.py
        ;;
  stop)
        # Stop daemons.
        echo "🦄 Shutting down $SERVICE_NAME";
        pid=`ps ax | grep -i '/standalone-uptime-export.py' | grep -v grep | awk '{print $1}'`
        if [ -n "$pid" ]
          then
          kill -9 $pid
         else
