...
SERVICE_NAME=es-curator-service-all-service
...
case "$1" in
  start)
        ...
        # nohup $ES_SERVICE_ALL_EXPORT_PATH/standalone-es-curator.py &> /dev/null &
        python $SCRIPTDIR/standalone-es-curator.py
        ;;
  stop)
        # Stop daemons.
        echo "🦄 Shutting down $SERVICE_NAME";
        pid=`ps ax | grep -i '/standalone-es-curator.py' | grep -v grep | awk '{print $1}'`
        if [ -n "$pid" ]
          then
          kill -9 $pid
         else
