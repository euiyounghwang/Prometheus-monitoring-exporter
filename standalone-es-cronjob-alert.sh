...
SERVICE_NAME=es-cronjob-service
...
case "$1" in
  start)
        ...
        # nohup $SCRIPTDIR/standalone-es-cronjob-alert.py &> /dev/null &
        python $SCRIPTDIR/standalone-es-cronjob-alert.py
        ;;
  stop)
        # Stop daemons.
        echo "🦄 Shutting down $SERVICE_NAME";
        pid=`ps ax | grep -i '/standalone-es-cronjob-alert.py' | grep -v grep | awk '{print $1}'`
        if [ -n "$pid" ]
          then
          kill -9 $pid
         else
