...
go run ./go_db.go -api_host $API_HOST -es_url $ES_URL -kibana_url $KIBANA_URL -kafka_url $KAFKA_URL -zookeeper_url $ZOOKEEPER_URL -spark_url $SPARK_URL -db_url $DB_URL -sql "SELECT * FROM TB"
