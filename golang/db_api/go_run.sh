...
export LOGSTASH_URL=$LOGSTASH_HOST:5043,$LOGSTASH_HOST:5044,$LOGSTASH_HOST:5045,$LOGSTASH_HOST:5046,$LOGSTASH_HOST:5047,$LOGSTASH_HOST:5048,$LOGSTASH_HOST:5144
...
go run ./go_db.go -env_name $SET_ENV_NAME -api_host $API_HOST -alert_conf_api $ALERT_CONFIGURATION_API -es_url $ES_URL -kibana_url $KIBANA_URL -logstash_url $LOGSTASH_URL -kafka_url $KAFKA_URL -zookeeper_url $ZOOKEEPER_URL -kafka_connect_url $KAFKA_CONNECT_URL -spark_url $SPARK_URL -db_url $DB_URL -sql "SELECT * FROM TB"
