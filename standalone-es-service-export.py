        ...
        for k, v in monitoring_metrics.items():
            ...
            for idx, each_host in enumerate(url_lists):
                ...
                try:
                    ...
                    # if 'redis' not in str(k) and 'configuration' not in str(k) and 'loki_custom_promtail_agent_url' not in str(k) and 'log_aggregation_agent_url' not in str(k):
                    if str(k) not in exclude_port_detect:
                        saved_failure_dict.update({each_urls[0] + "_" + str(k).upper() + "_" + str(idx+1): "[Node #{}-{}] ".format(idx+1, str(k).upper()) + each_host + f"{str(e)}"})
                    pass
            ...
            for each_es_host in es_url_hosts_list:
                ...
                try:
                    ...
                    for each_json_info in list(resp_info.json()):
                        ...
                        if each_json_info.get("index").startswith("wx_") or each_json_info.get("index").startswith("om_"):
                    ...
                    for element_dict in resp.json():
                        for k, v in element_dict.items():
                            ...
                            if float(element_dict.get("diskUsedPercent","-1")) >= int(disk_usage_threshold_es_config_api):
                                # nodes_diskspace_gauge_g.labels(server_job=domain_name_as_nick_name, category="Elastic Node", host="{}{}".format(str(global_env_name).lower(), get_host_name), name=element_dict.get("name",""), ip=element_dict.get("ip",""), disk_health=disk_es_health, disktotal=element_dict.get("diskTotal",""), diskused=element_dict.get("diskUsed",""), diskavail=element_dict.get("diskAvail",""), diskusedpercent=element_dict.get("diskUsedPercent","")+"%").set(0)
                                nodes_diskspace_gauge_g.labels(server_job=domain_name_as_nick_name, category="Elastic Node", host="{}{}".format(str(global_env_name).lower(), get_host_name), ip=element_dict.get("ip",""), disk_health=disk_es_health, disktotal=element_dict.get("diskTotal",""), diskused=element_dict.get("diskUsed",""), diskavail=element_dict.get("diskAvail",""), diskusedpercent=element_dict.get("diskUsedPercent","")+"%").set(0)
                            else:
                                # nodes_diskspace_gauge_g.labels(server_job=domain_name_as_nick_name, category="Elastic Node", host="{}{}".format(str(global_env_name).lower(), get_host_name), name=element_dict.get("name",""), ip=element_dict.get("ip",""), disk_health=disk_es_health, disktotal=element_dict.get("diskTotal",""), diskused=element_dict.get("diskUsed",""), diskavail=element_dict.get("diskAvail",""), diskusedpercent=element_dict.get("diskUsedPercent","")+"%").set(1)
                                nodes_diskspace_gauge_g.labels(server_job=domain_name_as_nick_name, category="Elastic Node", host="{}{}".format(str(global_env_name).lower(), get_host_name), ip=element_dict.get("ip",""), disk_health=disk_es_health, disktotal=element_dict.get("diskTotal",""), diskused=element_dict.get("diskUsed",""), diskavail=element_dict.get("diskAvail",""), diskusedpercent=element_dict.get("diskUsedPercent","")+"%").set(1)
...
def db_jobs_backlogs_work(interval, database_object, sql, db_http_host, db_url, db_info):
    ...
    while True:
        try:
            ...
            Max_Backlog_CNT = gloabl_configuration.get('config').get('backlog_threshold') if gloabl_configuration else 30000
